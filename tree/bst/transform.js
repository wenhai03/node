...
class BST { // binary search tree
  ...
  inorderTraversal () {
    const traversal = (node) => {
      ...
      traversal(node.left)
      console.log(node.element)
      traversal(node.right) // 左  中 右   |   右  中 左
    }
...
// 深度(先序遍历 中序遍历 后续遍历) 广度(层序)  都可以拿到树中的节点

// 尝试着用非递归遍历 (先序遍历  中序遍历 后序遍历) 实现一个深拷贝  {name: 1}

// bst.postorderTraversal() // babel中ast 语法树的遍历
